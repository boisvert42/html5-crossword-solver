...
function xw_read_ipuz(data) {
    ...
    */
    ...
    console.log(words);
    if (!words.length || data.fakeclues) {
        words = [];
        var thisGrid = new xwGrid(cells);
        var word_id = 1;
        ...
        // If "fakeclues", we disassociate clues from words
        if (data.fakeclues) {
          clues.forEach(function (clueGroup) {
            clueGroup['clue'].forEach(function (clue) {
              clue.word = '';
            });
          });
        }
            ...
            for (x = 0; x < this.width; x++) {
                if (this.startAcrossWord(x, y)) {
                    ...
                    if (!acrossEntries[thisNum] && thisNum) {
                        acrossEntries[thisNum] = {'word': '', 'id': thisNum.toString(), 'cells': []};
                    }
                }
            ...
            for (y = 0; y < this.height; y++) {
                if (this.startDownWord(x, y)) {
                    ...
                    if (!downEntries[thisNum] && thisNum) {
                        downEntries[thisNum] = {'word': '', 'id': (Number(thisNum) + 1000).toString(), 'cells': []};
                    }
                }
                if (!this.isBlack(x, y) && thisNum) {
                    var letter = this.letterAt(x, y);
